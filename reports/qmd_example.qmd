...
![Horse populations for all provinces in Canada from 1906 - 1972.](../results/horse_pops_plot.png){#fig-HorsePopPlot width=90%}

We can see from @fig-HorsePopPlot that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in @tbl-sdhorses.
...
Additionally, note that in @tbl-sdhorses we consider the sample standard deviation of the number of horses during the same time span as @fig-HorsePopPlot.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-largestprov width=70%}

In @fig-largestprov we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
